...
async function payoutPickupScheduledOrders() {
  ...
  const now = Date.now();
  // order must be AT LEAST 20 days old, but not yet 21 days old
  ...
  for (const order of orders) {
    try {
      ...
      const sellerShare = order.amount * (1 - PLATFORM_FEE_PERCENT / 100);
      const sellerShareCents = Math.round(sellerShare * 100); // Stripe wants integer cents
...
// Trigger via cron / scheduler hitting this route, e.g. GET /api/orders/track
